...
;(function(window) {
	...
	function listen() {
		...
		animatePath(shzPathEl, shzEl.getAttribute('data-path-listen'), 400, [0.8, -0.6, 0.2, 1], function() {
			...
			setTimeout(revertToListen, 10000);
			setTimeout(showPlayer, 20000);
		});
	}
	function revertToListen() {	
		stopListening();
		// morph the listening button shape back to the initial button shape
		animatePath(shzPathEl, shzEl.getAttribute('data-path-start'), 400, [0.4, 1, 0.3, 1], function() {
			// toggle classes (button content/text changes)
			classie.add(shzCtrl, 'button--animate');
			classie.remove(shzCtrl, 'button--listen');
			classie.add(shzCtrl, 'button--loading');
		});
	}
	/**
	...
	function showPlayer() {
		...
		classie.remove(shzCtrl, 'button--loading');
	}
